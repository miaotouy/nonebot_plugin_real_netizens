...
from .db.database import get_user, update_user
# 获取 logger 对象
import logging
...
async def handle_group_message(bot: Bot, event: GroupMessageEvent, state: T_State):
    # 保存用户信息到数据库
    await save_user_info(bot, event)
    ...
    if check_trigger(group_id, message, group_config):
        ...
        if await should_respond(message, character_config, chat_history):
            ...
            # 获取用户信息
            user = await get_user(get_session(), int(event.user_id))
            if user:
                context["user_nickname"] = user.nickname
                # 可以添加其他用户相关信息到 context 中
    ...
    user = await get_user(get_session(), int(event.user_id))
    if user:
        await update_user(get_session(), user, last_message=str(message), last_message_time=datetime.now())
